...
class MarketFetcher:
    ...
    def get_index_daily_price(self, save: bool = True, save_dir: str = None) -> [DailyPrice]:
        ...
        daily_price = self.strategies.index_price.get_daily_price(index_ticker)
        print(f'Fetched {len(daily_price)} {index_ticker} daily price(s) from '
              f'{self.strategies.index_price.get_source_name()}')
        if save:
            self.export(daily_price, f'INDEX_DAILY_{index_ticker.upper()}.csv', file_dir=save_dir)
        return daily_price
    ...
    def get_constituents_daily_price(self, ticker: str, save: bool = True, save_dir: str = None) -> [DailyPrice]:
        daily_price = self.strategies.constituents_price.get_daily_price(ticker)
    ...
    @staticmethod
    def export(data: List, file_name: str, file_dir: str = None):
        if data:
            file_path = pathlib.Path(file_dir or __file__).parents[1].joinpath(f'dataset/{file_name}')
